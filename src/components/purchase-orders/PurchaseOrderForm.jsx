...
export default function PurchaseOrderForm({ purchaseOrder, contacts, onSave, onCancel }) {
  ...
  const [formData, setFormData] = useState({
    ...
    issue_date: purchaseOrder?.issue_date || format(new Date(), 'yyyy-MM-dd'),
    delivery_date: purchaseOrder?.delivery_date || format(new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), 'yyyy-MM-dd'),
    line_items: purchaseOrder?.line_items || [{ description: "", quantity: 1, unit_price: 0, total: 0, account_id: "" }],
    notes: purchaseOrder?.notes || "",
    status: purchaseOrder?.status || "draft",
    organization_id: purchaseOrder?.organization_id || "default_org"
  });
  ...
  const handleSubmit = (e) => {
    ...
    onSave({ ...formData, ...totals, total_amount: totals.total });
  };
  ...
  return (
    <Card className="border-0 shadow-lg mb-8">
      <CardHeader><CardTitle>{purchaseOrder ? "Edit" : "New"} Purchase Order</CardTitle></CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid md:grid-cols-2 gap-6">
            <div><Label>Supplier</Label><Select value={formData.contact_id} onValueChange={handleContactChange} required><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{contacts.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div>
            <div><Label>PO Number</Label><Input value={formData.po_number} onChange={e => setFormData({...formData, po_number: e.target.value})} /></div>
            ...
            <div><Label>Delivery Date</Label><Input type="date" value={formData.delivery_date} onChange={e => setFormData({...formData, delivery_date: e.target.value})} /></div>
          </div>
          ...
          <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={onCancel}><X className="w-4 h-4 mr-2"/>Cancel</Button><Button type="submit"><Save className="w-4 h-4 mr-2"/>Save PO</Button></div>
        </form>
      </CardContent>
    </Card>
  );
